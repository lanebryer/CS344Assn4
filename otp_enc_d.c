...
//this funciton receives data in 1000 byte chunks until it finds the terminating symbol @@
//It then sends a confirmation (client blocks until it receives confirmation) so that client/server don't desync
...
//This function sends data until there is notthing left to send, and then waits to receive a confirmation from the client before moving on
...
//Small function to strip the terminating @@ symbol from he string
...
void encryptString(char plain[], char key[])
{
	int i;
	int encryptedChar;
	int plainVal, keyVal;
	...
	for(i = 0; i < (int)strlen(plain); i++)
	{
		...
		encryptedChar %= 27;
		encryptedText[i] = chars[encryptedChar];
	}
}
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		else
		{
			...
			encryptString(plainText, keyText);
			strcat(encryptedText, "@@");
			
			charsWritten = sendAll(establishedConnectionFD, encryptedText, strlen(encryptedText));
